...
export class UserService {
  ...
  async createUser(userData: Partial<User>): Promise<User> {
    ...
    const published = await publishToQueue('user_events', 'user.created', message);
    if (published) {
      console.log(`[user-service] Published user.created event for user ID: ${savedUser.user_id}`);
    } else {
      console.error(`[user-service] Failed to publish user.created event for user ID: ${savedUser.user_id}`);
    }
